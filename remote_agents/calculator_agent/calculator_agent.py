...
def calculator_tool(expression: str) -> str:
    """Tính toán biểu thức số học, ví dụ: '10 + 5 * 2'."""
    try:
        # Môi trường an toàn cho eval
        safe_dict = {}
    ...
    async def stream(self, query: str, context_id: str) -> AsyncGenerator[dict, None]:
        ...
        while True:
            ...
            if isinstance(last_message, AIMessage) and last_message.tool_calls:
                yield {'is_task_complete': False, 'content': "Đang sử dụng công cụ máy tính..."}
            elif isinstance(last_message, ToolMessage):
                yield {'is_task_complete': False, 'content': "Đang xử lý kết quả..."}
